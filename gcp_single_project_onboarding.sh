...
function vss_gcp_onboard() {
  ...
  gcloud iam service-accounts keys create --iam-account vmware-secure-state-account@$proj_id.iam.gserviceaccount.com vmw-secure-state-sa-key.json && l_ok=OK || l_ok=""
      if [ -z "$l_ok" ]; then
